...
class DatasetAozora(Dataset):
    ...
    def _make_datalist(self):
        ...
        for ctg in categories:
            q = f"分類番号 == '{ctg}'"
            cat_df = self.list_df.query(q).sample(
                self.n_threshold, random_state=self.rs
            )[columns]
    ...
    def get_labels(self, do_split=True):
        if do_split:
            assert self.train is not None
            assert self.valid is not None
            return [drec.label for drec in self.train], [
                drec.label for drec in self.valid
            ]
